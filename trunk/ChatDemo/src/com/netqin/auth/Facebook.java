...
import com.netqin.authprotocal.AsyncRunner;
import com.netqin.authprotocal.AuthorizationError;
import com.netqin.authprotocal.AuthorizationProtocol;
import com.netqin.authprotocal.BaseRequestListener;
import com.netqin.authprotocal.CustomDialog;
import com.netqin.authprotocal.DialogError;
import com.netqin.authprotocal.FacebookError;
import com.netqin.authprotocal.LoginListener;
import com.netqin.authprotocal.Util;
 ...
 */
public class Facebook implements AuthorizationProtocol {
	...
	private String mPicURL;// 头像url
	private String mNickName;// 用户昵称
	private String mUserUID;// 用户uid
	...
	private LoginListener mAuthDialogListener;
	private boolean mFacebookNeedReAuth = false;
	 ...
	 */
	public static Facebook getFacebookInstance(String appId) {
		if (sFacebook == null) {
	...
	public void authorize(Activity activity, final LoginListener listener) {
		authorize(activity, false, -1, new String[] {},
				DEFAULT_AUTH_ACTIVITY_CODE, listener);
	}
	...
	public void authorize(Activity activity, String[] permissions,
		...
		authorize(activity, false, -1, permissions, DEFAULT_AUTH_ACTIVITY_CODE,
				listener);
	}
	 ...
	 */
	@Override
	public void authorize(Activity activity, boolean facebookNeedReAuth,
			int dialogId, String[] permissions, int activityCode,
			final LoginListener listener) {
		...
		mDialogId = dialogId;
		mFacebookNeedReAuth = facebookNeedReAuth;
		String[] permission = new String[] { "xmpp_login" };
		startDialogAuth(activity, permission);// all in webview
	...
	private String getPictureUrl(JSONObject jsonObject) {
		// picture may contain another json object formed like
		// "picture":{"data":{"is_silhouette":false,"url":"http:\/\/profile.ak.fbcdn.net\/hprofile-ak-ash4\/369113_100004270664076_963094116_q.jpg"}}
		// "picture":""
		...
		try {
			final String kPictureString = "picture";
			url = jsonObject.getString(kPictureString);
			JSONObject t = jsonObject.getJSONObject(kPictureString);
			if (t != null) {
	...
	public class UserRequestListener extends BaseRequestListener {
		...
		public void onComplete(final String response, final Object state) {
			...
			try {
				...
				setUserUID(jsonObject.getString("id"));

				setPicUrl(getPictureUrl(jsonObject));
				setNickName(jsonObject.getString("name"));
				mAuthDialogListener.onComplete(null);
		...
		public void onFacebookError(FacebookError e, final Object state) {
			...
			e.printStackTrace();
			mAuthDialogListener.onFacebookError(new AuthorizationError(
	...
	public void requestUserData() {
		...
		AsyncRunner.getAsyncRunnerInstance(this).request("me", params,
	...
	private void startDialogAuth(Activity activity, String[] permissions) {
		...
		if (mFacebookNeedReAuth) {
			Util.clearCookies(activity);
		}
		dialog(activity, LOGIN, params, new LoginListener() {

			public void onComplete(Bundle values) {
				// ensure any cookies set by the dialog are saved
				CookieSyncManager.getInstance().sync();
				setAccessToken(values.getString(AuthorizationProtocol.TOKEN));
				setmSessionString(values.getString(AuthorizationProtocol.SESSION));
				setAccessExpiresIn(values
						.getString(AuthorizationProtocol.EXPIRES));
				if (isSessionValid()) {
					...
					if (mAuthActivity != null && mDialogId >= 0) {
						mAuthActivity.showDialog(mDialogId);
					}
					requestUserData();
				} else {
					mAuthDialogListener.onFacebookError(new AuthorizationError(
			...
			public void onFacebookError(AuthorizationError error) {
				Util.logd("Facebook-authorize", "Login failed: " + error);
				...
				if (error != null) {
					if (error.equals(SINGLE_SIGN_ON_DISABLED)
						...
						Util.logd("Facebook-authorize", "Login failed: "
								+ error);
						mAuthDialogListener
								.onFacebookError(new AuthorizationError(error));
					}
					...
					if (isSessionValid()) {
						...
						mAuthDialogListener
								.onFacebookError(new AuthorizationError(
										"Failed to receive access token."));
					}
				}
			...
			public void handleMessage(Message msg) {
				...
				if (token != null) {
					...
					if (msg.getData().containsKey("error_code")) {
						...
						serviceListener.onFacebookError(new AuthorizationError(
	 ...
	 *            has completed.
	 */
	public void dialog(Activity context, String action, LoginListener listener) {
	 ...
	 *            has completed.
	 */
	public void dialog(Activity context, String action, Bundle parameters,
			final LoginListener listener) {
		...
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			new CustomDialog(context, url, listener).show();
		}
	}
	...
	public String getmSessionString() {
		return mSessionString;
	}

	public void setmSessionString(String mSessionString) {
		this.mSessionString = mSessionString;
	}
	/**
